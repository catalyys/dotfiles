...
-- Disable LSP server prompts
-- vim.lsp.set_log_level("off") -- Optional: reduces LSP logging

-- Override the handler that shows the prompt
vim.lsp.handlers["window/showMessageRequest"] = function(_, result, ctx)
  -- Auto-select the first option or handle based on filetype
  local filetype = vim.bo.filetype

  if result.type == 3 then -- Info message
    if filetype == "yaml.ansible" then
      -- For ansible files, choose ansiblels if available
      if string.find(result.message:lower(), "multiple") then
        return 0 -- Select first option
      end
    end
  end

  -- Default: use the first option
  return 0
end
